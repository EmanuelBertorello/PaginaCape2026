import { CiudadData, ProvinciaData } from '../models/interfaces';

export const CIUDADES_SANTA_FE: CiudadData[] = [
  {
    slug: 'rosario',
    nombre: 'Rosario',
    provincia: 'Santa Fe',
    provinciaSlug: 'santa-fe',
    descripcion: 'Rosario es el principal polo industrial de Santa Fe. Con alta concentración en frigoríficos, astilleros y construcción, los accidentes laborales son frecuentes.',
    poblacion: '1.200.000',
    sectorPrincipal: 'industria, construcción, salud',
    cmLocal: 'CM N° 2 — Rosario',
    cmSlug: 'comision-medica-rosario',
    metaTitle: 'Abogados Accidentes Laborales Rosario | Capeletti',
    metaDescription: 'Abogados especialistas en accidentes laborales y ART en Rosario. Comisión Médica local N° 2. Consulta gratis, sin anticipos. Capeletti Abogados.',
    industriasPrincipales: ['Frigoríficos y alimentación', 'Astilleros y metalurgia', 'Construcción civil', 'Salud y enfermería', 'Logística y transporte'],
    lesionesComunes: ['Hernia de disco lumbar', 'Lesiones de manguito rotador', 'Hipoacusia laboral', 'Accidentes in itinere', 'Aplastamientos en líneas de producción'],
    parrafoSeo: 'Rosario concentra más de 200 plantas industriales con alta siniestralidad: frigoríficos sobre el cordón industrial, astilleros en la costa del Paraná y grandes obras de construcción civil. El alto volumen de accidentes en estos sectores tiene su correlato en la Comisión Médica N°2, que es una de las más activas del país. Nuestros abogados conocen personalmente a los médicos peritos, los plazos reales del trámite y las estrategias más efectivas para obtener el mayor porcentaje de incapacidad ante esa CM.',
    jurisdiccion: 'Cámara de Apelaciones Laboral de Rosario — 2° Circunscripción Judicial de Santa Fe',
  },
  {
    slug: 'santa-fe-capital',
    nombre: 'Santa Fe Capital',
    provincia: 'Santa Fe',
    provinciaSlug: 'santa-fe',
    descripcion: 'Capital provincial con fuerte presencia en construcción, administración pública y servicios. Sede de la Comisión Médica local.',
    poblacion: '400.000',
    sectorPrincipal: 'construcción, administración pública, salud',
    cmLocal: 'CM N° 1 — Santa Fe',
    cmSlug: 'comision-medica-santa-fe',
    metaTitle: 'Abogados Accidentes Laborales Santa Fe Capital | Capeletti',
    metaDescription: 'Abogados en Santa Fe Capital para reclamos laborales y ART. Comisión Médica N° 1. Sin honorarios anticipados. Consultá gratis hoy.',
    industriasPrincipales: ['Administración pública y docencia', 'Construcción y obra pública', 'Salud y hospitales provinciales', 'Comercio y servicios'],
    lesionesComunes: ['Lumbalgia crónica', 'Estrés laboral y enfermedades psíquicas', 'Caídas en altura en obra', 'Accidentes de tránsito in itinere'],
    parrafoSeo: 'Santa Fe Capital alberga los principales organismos estatales provinciales y un sector de la construcción muy activo. Los trabajadores del Estado —tanto docentes como personal de salud— tienen alta prevalencia de enfermedades profesionales ligadas a esfuerzo físico y estrés laboral. Nuestro estudio tramita ante la Comisión Médica N°1 con representación directa sin necesidad de desplazamientos del trabajador.',
    jurisdiccion: 'Cámara de Apelaciones Laboral de Santa Fe — 1° Circunscripción Judicial',
  },
  {
    slug: 'reconquista',
    nombre: 'Reconquista',
    provincia: 'Santa Fe',
    provinciaSlug: 'santa-fe',
    descripcion: 'Reconquista tiene fuerte actividad agroindustrial y forestal. La Comisión Médica local atiende a toda la zona norte de Santa Fe.',
    poblacion: '80.000',
    sectorPrincipal: 'agroindustria, forestal, construcción',
    cmLocal: 'CM N° 40C — Reconquista',
    cmSlug: 'comision-medica-reconquista',
    metaTitle: 'Abogados Accidentes Laborales Reconquista | Capeletti',
    metaDescription: 'Abogados especializados en ART y accidentes laborales en Reconquista, Santa Fe. Comisión Médica local. Consulta gratis.',
  },
  {
    slug: 'rafaela',
    nombre: 'Rafaela',
    provincia: 'Santa Fe',
    provinciaSlug: 'santa-fe',
    descripcion: 'Rafaela es el centro industrial más importante del centro-oeste santafesino, con fuerte presencia en metalurgia y alimentación.',
    poblacion: '105.000',
    sectorPrincipal: 'metalurgia, alimentación, textil',
    cmLocal: 'CM — Rafaela',
    cmSlug: 'comision-medica-rafaela',
    metaTitle: 'Abogados Laborales Rafaela | Capeletti Abogados',
    metaDescription: 'Abogados en Rafaela para reclamos a la ART y accidentes de trabajo. Casos de metalurgia y alimentación. Consulta gratis sin costo.',
  },
  {
    slug: 'venado-tuerto',
    nombre: 'Venado Tuerto',
    provincia: 'Santa Fe',
    provinciaSlug: 'santa-fe',
    descripcion: 'Venado Tuerto es el eje del sur provincial con importante actividad agroindustrial y de logística.',
    poblacion: '80.000',
    sectorPrincipal: 'agroindustria, logística, construcción',
    cmLocal: 'CM — Venado Tuerto',
    cmSlug: 'comision-medica-venado-tuerto',
    metaTitle: 'Abogados Laborales Venado Tuerto | Capeletti',
    metaDescription: 'Abogados especializados en accidentes laborales en Venado Tuerto. Reclamos a la ART con representación local. Consulta gratis.',
  },
];

export const CIUDADES_NEUQUEN: CiudadData[] = [
  {
    slug: 'neuquen-capital',
    nombre: 'Neuquén Capital',
    provincia: 'Neuquén',
    provinciaSlug: 'neuquen',
    descripcion: 'Neuquén capital es el principal centro de la Patagonia norte con fuerte actividad petrolera, construcción y servicios.',
    poblacion: '350.000',
    sectorPrincipal: 'petróleo, construcción, servicios',
    cmLocal: 'CM N° 10 — Neuquén',
    cmSlug: 'comision-medica-neuquen',
    metaTitle: 'Abogados Accidentes Laborales Neuquén | Capeletti',
    metaDescription: 'Abogados especialistas en ART y accidentes laborales en Neuquén. Industria petrolera, construcción y más. CM local N°10. Consulta gratis.',
    industriasPrincipales: ['Extracción de petróleo y gas — Vaca Muerta', 'Construcción de pozos y plantas', 'Generación hidroeléctrica', 'Comercio y servicios patagónicos'],
    lesionesComunes: ['Quemaduras e intoxicaciones por gases', 'Caídas en pozos y plataformas', 'Lesiones de columna por maquinaria pesada', 'Accidentes en transporte de personal'],
    parrafoSeo: 'El boom de Vaca Muerta convirtió a Neuquén en uno de los mercados laborales de mayor siniestralidad del país. El sector petrolero —regulado por el CCT 644/12— expone a miles de trabajadores a accidentes de alta gravedad: derrames, explosiones, caídas y aplastamientos. Capeletti Abogados tiene matrícula activa en el Colegio de Abogados de Neuquén (N° 4452) y tramita directamente ante la CM N°10 con conocimiento profundo de las particularidades del sector hidrocarburífero.',
    jurisdiccion: 'Cámara Provincial del Trabajo de Neuquén',
  },
  {
    slug: 'zapala',
    nombre: 'Zapala',
    provincia: 'Neuquén',
    provinciaSlug: 'neuquen',
    descripcion: 'Zapala es un nodo importante de la actividad minera y de logística en el interior de Neuquén.',
    poblacion: '45.000',
    sectorPrincipal: 'minería, logística, agroindustria',
    cmLocal: 'CM — Zapala',
    metaTitle: 'Abogados Laborales Zapala Neuquén | Capeletti',
    metaDescription: 'Abogados para reclamos a la ART en Zapala, Neuquén. Minería y accidentes laborales. Sin anticipos. Consulta gratis.',
  },
  {
    slug: 'plaza-huincul',
    nombre: 'Plaza Huincul',
    provincia: 'Neuquén',
    provinciaSlug: 'neuquen',
    descripcion: 'Plaza Huincul y Cutral Có conforman el corazón de la actividad petrolera del interior neuquino. Alta siniestralidad en extracción de crudo y gas.',
    poblacion: '17.000',
    sectorPrincipal: 'petróleo, gas, minería',
    cmLocal: 'CM — Plaza Huincul',
    cmSlug: 'comision-medica-plaza-huincul',
    metaTitle: 'Abogados Laborales Plaza Huincul | Capeletti',
    metaDescription: 'Abogados especializados en accidentes laborales en Plaza Huincul, Neuquén. Industria petrolera y gasífera. Consulta gratis.',
  },
];

export const CIUDADES_RIO_NEGRO: CiudadData[] = [
  {
    slug: 'bariloche',
    nombre: 'San Carlos de Bariloche',
    provincia: 'Río Negro',
    provinciaSlug: 'rio-negro',
    descripcion: 'Bariloche tiene fuerte actividad en turismo, gastronomía y construcción. Los accidentes laborales son frecuentes en temporada alta.',
    poblacion: '130.000',
    sectorPrincipal: 'turismo, construcción, gastronomía',
    cmLocal: 'CM — Bariloche',
    cmSlug: 'comision-medica-bariloche',
    metaTitle: 'Abogados Laborales Bariloche | Capeletti Abogados',
    metaDescription: 'Abogados especializados en accidentes laborales en Bariloche, Río Negro. Gastronomía, turismo y construcción. Consulta gratis.',
    industriasPrincipales: ['Hotelería y gastronomía turística', 'Construcción y obra privada', 'Fruticultura y viveros', 'Transporte de pasajeros'],
    lesionesComunes: ['Caídas en cocinas y hoteles', 'Lesiones de espalda en obra', 'Accidentes en estaciones de esquí', 'Accidentes in itinere en ruta'],
    parrafoSeo: 'La economía de Bariloche gira en torno al turismo, lo que implica alta rotación laboral y muchos trabajadores estacionales con cobertura de ART. En temporada alta, la industria gastronómica y hotelera concentra un porcentaje importante de los accidentes. Capeletti Abogados tramita ante la Comisión Médica de Bariloche con matrícula activa en Río Negro (N° 6636).',
    jurisdiccion: 'Cámara Laboral de Río Negro — sede Bariloche',
  },
  {
    slug: 'viedma',
    nombre: 'Viedma',
    provincia: 'Río Negro',
    provinciaSlug: 'rio-negro',
    descripcion: 'Viedma es la capital de Río Negro, con actividad en administración pública, agroindustria y construcción.',
    poblacion: '70.000',
    sectorPrincipal: 'administración pública, agroindustria',
    cmLocal: 'CM — Viedma',
    cmSlug: 'comision-medica-viedma',
    metaTitle: 'Abogados Accidentes Laborales Viedma | Capeletti',
    metaDescription: 'Abogados para reclamos laborales y ART en Viedma, Río Negro. Comisión Médica local. Consulta gratis y sin costo anticipado.',
  },
  {
    slug: 'general-roca',
    nombre: 'General Roca',
    provincia: 'Río Negro',
    provinciaSlug: 'rio-negro',
    descripcion: 'General Roca es el principal centro frutícola y agroindustrial de Río Negro. Alta siniestralidad en cosecha, empaque y agroindustria.',
    poblacion: '90.000',
    sectorPrincipal: 'fruticultura, agroindustria, petróleo',
    cmLocal: 'CM — General Roca',
    cmSlug: 'comision-medica-general-roca',
    metaTitle: 'Abogados Laborales General Roca | Capeletti Abogados',
    metaDescription: 'Abogados especializados en accidentes laborales en General Roca, Río Negro. Fruticultura y agroindustria. Consulta gratis.',
  },
  {
    slug: 'cipolletti',
    nombre: 'Cipolletti',
    provincia: 'Río Negro',
    provinciaSlug: 'rio-negro',
    descripcion: 'Cipolletti concentra una importante actividad frutícola y comercial en el Alto Valle de Río Negro. Frecuentes accidentes en empaque y cosecha.',
    poblacion: '90.000',
    sectorPrincipal: 'fruticultura, comercio, construcción',
    cmLocal: 'CM — Cipolletti',
    cmSlug: 'comision-medica-cipolletti',
    metaTitle: 'Abogados Laborales Cipolletti | Capeletti Abogados',
    metaDescription: 'Abogados para reclamos a la ART en Cipolletti, Río Negro. Fruticultura y agroindustria. Consulta gratis sin anticipos.',
  },
];

export const CIUDADES_ENTRE_RIOS: CiudadData[] = [
  {
    slug: 'parana',
    nombre: 'Paraná',
    provincia: 'Entre Ríos',
    provinciaSlug: 'entre-rios',
    descripcion: 'Paraná es la capital entrerriana con actividad en construcción, salud y administración pública.',
    poblacion: '250.000',
    sectorPrincipal: 'construcción, salud, administración pública',
    cmLocal: 'CM N° 5 — Paraná',
    cmSlug: 'comision-medica-parana',
    metaTitle: 'Abogados Laborales Paraná Entre Ríos | Capeletti',
    metaDescription: 'Abogados especializados en ART y accidentes laborales en Paraná, Entre Ríos. CM N°5 local. Consulta gratis.',
    industriasPrincipales: ['Avicultura y frigoríficos de aves', 'Administración pública y salud', 'Construcción civil e infraestructura', 'Citricultura y agroindustria'],
    lesionesComunes: ['Lesiones de columna por carga en frigoríficos', 'Hipoacusia laboral por maquinaria', 'Accidentes en procesadoras de aves', 'Lumbalgia crónica en operarios'],
    parrafoSeo: 'Entre Ríos tiene una de las industrias avícolas más grandes de Sudamérica, con frigoríficos de aves que son foco frecuente de accidentes laborales y enfermedades profesionales. El Colegio de la Abogacía de Entre Ríos (matrícula N° 11664) nos habilita a litigar directamente en la jurisdicción sin intermediarios, con representación ante la CM N°5 de Paraná.',
    jurisdiccion: 'Cámara del Trabajo de Paraná — Provincia de Entre Ríos',
  },
  {
    slug: 'concordia',
    nombre: 'Concordia',
    provincia: 'Entre Ríos',
    provinciaSlug: 'entre-rios',
    descripcion: 'Concordia tiene fuerte actividad citrícola y agroindustrial, con alta siniestralidad en temporadas de cosecha.',
    poblacion: '155.000',
    sectorPrincipal: 'citricultura, agroindustria, logística',
    cmLocal: 'CM — Concordia',
    cmSlug: 'comision-medica-concordia',
    metaTitle: 'Abogados Laborales Concordia | Capeletti Abogados',
    metaDescription: 'Abogados para reclamos a la ART en Concordia, Entre Ríos. Accidentes en citricultura y agroindustria. Consulta gratis.',
  },
  {
    slug: 'concepcion-del-uruguay',
    nombre: 'Concepción del Uruguay',
    provincia: 'Entre Ríos',
    provinciaSlug: 'entre-rios',
    descripcion: 'Concepción del Uruguay combina industria frigorífica, agroindustria y logística portuaria. Alta siniestralidad en frigoríficos y construcción.',
    poblacion: '80.000',
    sectorPrincipal: 'frigoríficos, agroindustria, logística',
    cmLocal: 'CM — Concepción del Uruguay',
    cmSlug: 'comision-medica-concepcion-del-uruguay',
    metaTitle: 'Abogados Laborales Concepción del Uruguay | Capeletti',
    metaDescription: 'Abogados para reclamos a la ART en Concepción del Uruguay, Entre Ríos. Frigoríficos y agroindustria. Consulta gratis.',
  },
];

export const CIUDADES_BUENOS_AIRES: CiudadData[] = [
  {
    slug: 'caba',
    nombre: 'Ciudad Autónoma de Buenos Aires',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'CABA concentra la mayor cantidad de trabajadores del país. Accidentes laborales en construcción, gastronomía, logística y salud.',
    poblacion: '3.100.000',
    sectorPrincipal: 'construcción, salud, gastronomía, logística',
    cmLocal: 'CM N° 1 — CABA',
    cmSlug: 'comision-medica-caba',
    metaTitle: 'Abogados Accidentes Laborales CABA | Capeletti',
    metaDescription: 'Abogados especialistas en accidentes laborales y ART en CABA. Sin anticipos. Consulta gratis con abogado especializado.',
    industriasPrincipales: ['Construcción civil y obras en altura', 'Gastronomía y hotelería', 'Salud (hospitales y clínicas)', 'Logística y depósitos', 'Seguridad privada'],
    lesionesComunes: ['Caídas en altura en construcción', 'Lesiones de columna en enfermería y salud', 'Accidentes de moto en reparto y delivery', 'Accidentes in itinere en transporte público', 'Cortes y aplastamientos en gastronomía'],
    parrafoSeo: 'Con sede en Lavalle 1506 Piso 1 Of. 6 (CABA), Capeletti Abogados atiende casos en Buenos Aires con matrícula activa en el C.P.A.C.F. (T° 148 F° 548). La CM N°1 de CABA es una de las más voluminosas del sistema, y la Justicia Nacional del Trabajo resuelve los juicios laborales en plazos generalmente más breves que en el interior. Nuestro equipo conoce los tribunales locales y actúa con representación directa.',
    jurisdiccion: 'Cámara Nacional de Apelaciones del Trabajo — Justicia Nacional del Trabajo (CABA)',
  },
  {
    slug: 'bahia-blanca',
    nombre: 'Bahía Blanca',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Bahía Blanca tiene un importante polo petroquímico y portuario. Alta siniestralidad en la industria química y de construcción.',
    poblacion: '300.000',
    sectorPrincipal: 'petroquímica, puerto, construcción',
    cmLocal: 'CM — Bahía Blanca',
    cmSlug: 'comision-medica-bahia-blanca',
    metaTitle: 'Abogados Laborales Bahía Blanca | Capeletti Abogados',
    metaDescription: 'Abogados para reclamos de ART en Bahía Blanca. Industria petroquímica y accidentes laborales. Consulta gratis sin anticipos.',
  },
  {
    slug: 'mar-del-plata',
    nombre: 'Mar del Plata',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Mar del Plata tiene actividad en pesca, turismo y construcción. Frecuentes accidentes in itinere y en industria pesquera.',
    poblacion: '650.000',
    sectorPrincipal: 'pesca, turismo, construcción',
    cmLocal: 'CM — Mar del Plata',
    cmSlug: 'comision-medica-mar-del-plata',
    metaTitle: 'Abogados Laborales Mar del Plata | Capeletti',
    metaDescription: 'Abogados especializados en ART y accidentes de trabajo en Mar del Plata. Industria pesquera y turismo. Consulta gratis.',
  },
  {
    slug: 'la-plata',
    nombre: 'La Plata',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'La Plata es la capital de la provincia de Buenos Aires, con fuerte actividad en administración pública, salud, educación y construcción.',
    poblacion: '800.000',
    sectorPrincipal: 'administración pública, salud, construcción',
    cmLocal: 'CM N° 36 — La Plata',
    cmSlug: 'comision-medica-la-plata',
    metaTitle: 'Abogados Accidentes Laborales La Plata | Capeletti',
    metaDescription: 'Abogados especializados en accidentes laborales y ART en La Plata. CM N° 36 local. Consulta gratis, sin anticipos.',
  },
  {
    slug: 'ramos-mejia',
    nombre: 'Ramos Mejía',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Ramos Mejía es el centro comercial y de servicios del partido de La Matanza, con alta densidad laboral en comercio, logística y construcción.',
    poblacion: '190.000',
    sectorPrincipal: 'comercio, logística, construcción',
    cmLocal: 'CM — Ramos Mejía (GBA Oeste)',
    cmSlug: 'comision-medica-ramos-mejia',
    metaTitle: 'Abogados Laborales Ramos Mejía | Capeletti',
    metaDescription: 'Abogados para reclamos a la ART en Ramos Mejía, Buenos Aires. Accidentes laborales en GBA Oeste. Consulta gratis.',
  },
  {
    slug: 'moron',
    nombre: 'Morón',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Morón es un importante centro urbano del GBA Oeste con actividad industrial, comercial y de transporte. Alta incidencia de accidentes en manufactura.',
    poblacion: '325.000',
    sectorPrincipal: 'industria, comercio, transporte',
    cmLocal: 'CM — Morón (GBA Oeste)',
    cmSlug: 'comision-medica-moron',
    metaTitle: 'Abogados Laborales Morón Buenos Aires | Capeletti',
    metaDescription: 'Abogados especializados en accidentes laborales y ART en Morón, GBA. Industria y transporte. Consulta gratis.',
  },
  {
    slug: 'moreno',
    nombre: 'Moreno',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Moreno concentra actividad en logística, textil y construcción. Una de las zonas del GBA con mayor siniestralidad laboral en depósitos y manufactura.',
    poblacion: '550.000',
    sectorPrincipal: 'logística, textil, construcción',
    cmLocal: 'CM — Moreno (GBA Oeste)',
    cmSlug: 'comision-medica-moreno',
    metaTitle: 'Abogados Laborales Moreno Buenos Aires | Capeletti',
    metaDescription: 'Abogados para reclamos a la ART en Moreno, GBA Oeste. Logística, textil y construcción. Consulta gratis sin anticipos.',
  },
  {
    slug: 'san-isidro',
    nombre: 'San Isidro',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'San Isidro es un distrito del GBA Norte con actividad en construcción, gastronomía, servicios y zona portuaria fluvial.',
    poblacion: '295.000',
    sectorPrincipal: 'construcción, gastronomía, servicios',
    cmLocal: 'CM — San Isidro (GBA Norte)',
    cmSlug: 'comision-medica-san-isidro',
    metaTitle: 'Abogados Laborales San Isidro | Capeletti Abogados',
    metaDescription: 'Abogados especializados en ART y accidentes laborales en San Isidro, Buenos Aires. Consulta gratis.',
  },
  {
    slug: 'general-san-martin',
    nombre: 'General San Martín',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'General San Martín concentra un importante parque industrial con metalurgia, química y transporte. Alta siniestralidad en industria pesada.',
    poblacion: '430.000',
    sectorPrincipal: 'industria, metalurgia, transporte',
    cmLocal: 'CM — General San Martín (GBA)',
    cmSlug: 'comision-medica-san-martin',
    metaTitle: 'Abogados Laborales General San Martín | Capeletti',
    metaDescription: 'Abogados para reclamos de ART en General San Martín, Buenos Aires. Industria y metalurgia. Consulta gratis.',
  },
  {
    slug: 'lomas-de-zamora',
    nombre: 'Lomas de Zamora',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Lomas de Zamora es uno de los partidos más poblados del GBA Sur, con actividad industrial, comercial y de salud.',
    poblacion: '620.000',
    sectorPrincipal: 'industria, comercio, salud',
    cmLocal: 'CM — Lomas de Zamora (GBA Sur)',
    cmSlug: 'comision-medica-lomas-de-zamora',
    metaTitle: 'Abogados Laborales Lomas de Zamora | Capeletti',
    metaDescription: 'Abogados especializados en accidentes laborales y ART en Lomas de Zamora. GBA Sur. Consulta gratis.',
  },
  {
    slug: 'lanus',
    nombre: 'Lanús',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Lanús tiene una larga tradición industrial con metalurgia, construcción y manufactura. Alta siniestralidad en planta y obras.',
    poblacion: '455.000',
    sectorPrincipal: 'metalurgia, construcción, industria',
    cmLocal: 'CM — Lanús (GBA Sur)',
    cmSlug: 'comision-medica-lanus',
    metaTitle: 'Abogados Laborales Lanús Buenos Aires | Capeletti',
    metaDescription: 'Abogados para reclamos a la ART en Lanús, GBA Sur. Metalurgia y accidentes industriales. Consulta gratis.',
  },
  {
    slug: 'quilmes',
    nombre: 'Quilmes',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Quilmes combina industria cervecera, vidriería y logística. Frecuentes accidentes laborales en planta industrial y transporte.',
    poblacion: '580.000',
    sectorPrincipal: 'industria, logística, construcción',
    cmLocal: 'CM — Quilmes (GBA Sur)',
    cmSlug: 'comision-medica-quilmes',
    metaTitle: 'Abogados Laborales Quilmes Buenos Aires | Capeletti',
    metaDescription: 'Abogados especializados en ART y accidentes laborales en Quilmes, GBA Sur. Consulta gratis sin anticipos.',
  },
  {
    slug: 'ezeiza',
    nombre: 'Ezeiza',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Ezeiza concentra actividad aeroportuaria, logística y de construcción. Alta siniestralidad en depósitos, carga aérea y obras viales.',
    poblacion: '190.000',
    sectorPrincipal: 'logística, aeronáutica, construcción',
    cmLocal: 'CM — Ezeiza (GBA Sur)',
    cmSlug: 'comision-medica-ezeiza',
    metaTitle: 'Abogados Laborales Ezeiza Buenos Aires | Capeletti',
    metaDescription: 'Abogados para reclamos a la ART en Ezeiza. Logística aeroportuaria y construcción. Consulta gratis.',
  },
  {
    slug: 'pergamino',
    nombre: 'Pergamino',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Pergamino es el centro agroindustrial del norte bonaerense, con fuerte actividad en semillas, construcción y logística.',
    poblacion: '112.000',
    sectorPrincipal: 'agroindustria, semillas, construcción',
    cmLocal: 'CM — Pergamino',
    cmSlug: 'comision-medica-pergamino',
    metaTitle: 'Abogados Laborales Pergamino | Capeletti Abogados',
    metaDescription: 'Abogados especializados en ART y accidentes laborales en Pergamino, Buenos Aires. Agroindustria y construcción. Consulta gratis.',
  },
  {
    slug: 'junin',
    nombre: 'Junín',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Junín es un nodo agropecuario y comercial del noroeste bonaerense, con actividad en silos, transporte y construcción.',
    poblacion: '88.000',
    sectorPrincipal: 'agroindustria, comercio, transporte',
    cmLocal: 'CM — Junín',
    cmSlug: 'comision-medica-junin',
    metaTitle: 'Abogados Laborales Junín Buenos Aires | Capeletti',
    metaDescription: 'Abogados para reclamos de ART en Junín, Buenos Aires. Agroindustria y accidentes de trabajo. Consulta gratis.',
  },
  {
    slug: 'zarate',
    nombre: 'Zárate',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Zárate tiene un importante polo petroquímico, metalúrgico y papelero sobre el Paraná. Alta siniestralidad en planta industrial y logística portuaria.',
    poblacion: '120.000',
    sectorPrincipal: 'petroquímica, metalurgia, papelera',
    cmLocal: 'CM — Zárate',
    cmSlug: 'comision-medica-zarate',
    metaTitle: 'Abogados Laborales Zárate Buenos Aires | Capeletti',
    metaDescription: 'Abogados especializados en accidentes laborales en Zárate. Petroquímica, metalurgia y papelera. Consulta gratis.',
  },
  {
    slug: 'saladillo',
    nombre: 'Saladillo',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Saladillo es un centro ganadero y agroindustrial del centro bonaerense, con actividad en frigoríficos, campo y construcción.',
    poblacion: '35.000',
    sectorPrincipal: 'ganadería, agroindustria, construcción',
    cmLocal: 'CM — Saladillo',
    cmSlug: 'comision-medica-saladillo',
    metaTitle: 'Abogados Laborales Saladillo Buenos Aires | Capeletti',
    metaDescription: 'Abogados para reclamos a la ART en Saladillo, Buenos Aires. Ganadería y agroindustria. Consulta gratis.',
  },
  {
    slug: 'tandil',
    nombre: 'Tandil',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Tandil combina industria metalmecánica, turismo serrano y agroindustria. Frecuentes accidentes en manufactura y construcción.',
    poblacion: '128.000',
    sectorPrincipal: 'metalmecánica, turismo, construcción',
    cmLocal: 'CM — Tandil',
    cmSlug: 'comision-medica-tandil',
    metaTitle: 'Abogados Laborales Tandil Buenos Aires | Capeletti',
    metaDescription: 'Abogados especializados en ART y accidentes laborales en Tandil. Metalmecánica y construcción. Consulta gratis.',
  },
  {
    slug: 'trenque-lauquen',
    nombre: 'Trenque Lauquen',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Trenque Lauquen es el centro agroindustrial del oeste bonaerense, con actividad en ganadería, soja y logística de granos.',
    poblacion: '45.000',
    sectorPrincipal: 'agroindustria, ganadería, logística',
    cmLocal: 'CM — Trenque Lauquen',
    cmSlug: 'comision-medica-trenque-lauquen',
    metaTitle: 'Abogados Laborales Trenque Lauquen | Capeletti',
    metaDescription: 'Abogados para reclamos de ART en Trenque Lauquen, Buenos Aires. Agroindustria y ganadería. Consulta gratis.',
  },
  {
    slug: 'dolores',
    nombre: 'Dolores',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Dolores es el centro ganadero de la pampa deprimida bonaerense, con actividad en estancias, frigoríficos y turismo rural.',
    poblacion: '30.000',
    sectorPrincipal: 'ganadería, agroindustria, turismo rural',
    cmLocal: 'CM — Dolores',
    cmSlug: 'comision-medica-dolores',
    metaTitle: 'Abogados Laborales Dolores Buenos Aires | Capeletti',
    metaDescription: 'Abogados especializados en accidentes laborales en Dolores, Buenos Aires. Ganadería y campo. Consulta gratis.',
  },
  {
    slug: 'azul',
    nombre: 'Azul',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Azul tiene actividad en cemento, agroindustria y construcción. La industria cementera concentra parte importante de la siniestralidad local.',
    poblacion: '60.000',
    sectorPrincipal: 'cemento, agroindustria, construcción',
    cmLocal: 'CM — Azul',
    cmSlug: 'comision-medica-azul',
    metaTitle: 'Abogados Laborales Azul Buenos Aires | Capeletti',
    metaDescription: 'Abogados para reclamos a la ART en Azul, Buenos Aires. Cemento, agroindustria y construcción. Consulta gratis.',
  },
  {
    slug: 'pinamar',
    nombre: 'Pinamar',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Pinamar concentra actividad turística y de construcción estacional. Frecuentes accidentes laborales en temporada alta, gastronomía y obras.',
    poblacion: '30.000',
    sectorPrincipal: 'turismo, gastronomía, construcción',
    cmLocal: 'CM — Pinamar',
    cmSlug: 'comision-medica-pinamar',
    metaTitle: 'Abogados Laborales Pinamar Buenos Aires | Capeletti',
    metaDescription: 'Abogados especializados en ART y accidentes laborales en Pinamar. Turismo y construcción. Consulta gratis.',
  },
  {
    slug: 'necochea',
    nombre: 'Necochea',
    provincia: 'Buenos Aires',
    provinciaSlug: 'buenos-aires',
    descripcion: 'Necochea combina industria pesquera, agroindustria y turismo costero. Alta siniestralidad en plantas de procesamiento de pesca.',
    poblacion: '90.000',
    sectorPrincipal: 'pesca, agroindustria, turismo',
    cmLocal: 'CM — Necochea',
    cmSlug: 'comision-medica-necochea',
    metaTitle: 'Abogados Laborales Necochea Buenos Aires | Capeletti',
    metaDescription: 'Abogados para reclamos de ART en Necochea. Pesca, agroindustria y turismo. Consulta gratis sin anticipos.',
  },
];

export const CIUDADES_CORDOBA: CiudadData[] = [
  {
    slug: 'cordoba-capital',
    nombre: 'Córdoba Capital',
    provincia: 'Córdoba',
    provinciaSlug: 'cordoba',
    descripcion: 'Córdoba Capital concentra industria automotriz, construcción y servicios. Alta incidencia de accidentes laborales en manufactura y logística.',
    poblacion: '1.500.000',
    sectorPrincipal: 'automotriz, construcción, logística',
    cmLocal: 'CM N° 7 — Córdoba',
    cmSlug: 'comision-medica-cordoba',
    metaTitle: 'Abogados Accidentes Laborales Córdoba | Capeletti',
    metaDescription: 'Abogados especialistas en accidentes laborales y ART en Córdoba. CM local N° 7. Consulta gratis, sin anticipos.',
    industriasPrincipales: ['Industria automotriz (FIAT, Renault) — CCT SMATA', 'Construcción civil y vial', 'Alimentación y frigoríficos', 'Logística y transporte', 'Metalurgia y metalmecánica'],
    lesionesComunes: ['Aplastamientos en líneas de ensamblado', 'Lesiones de columna y extremidades en manufactura', 'Hipoacusia por ruido industrial', 'Accidentes con maquinaria pesada', 'Quemaduras en soldadura'],
    parrafoSeo: 'Córdoba es la segunda ciudad más grande del país y sede de las plantas automotrices más importantes de Argentina, reguladas bajo el CCT SMATA. El cordón industrial sur concentra alta siniestralidad en manufactura y logística. Capeletti Abogados representa trabajadores ante la CM N°7 de Córdoba, con conocimiento específico de los convenios colectivos del sector automotriz.',
    jurisdiccion: 'Cámara del Trabajo de la Ciudad de Córdoba',
  },
  {
    slug: 'rio-cuarto',
    nombre: 'Río Cuarto',
    provincia: 'Córdoba',
    provinciaSlug: 'cordoba',
    descripcion: 'Río Cuarto es el segundo centro urbano de Córdoba, con fuerte actividad agroindustrial y comercial.',
    poblacion: '175.000',
    sectorPrincipal: 'agroindustria, comercio',
    cmLocal: 'CM — Río Cuarto',
    cmSlug: 'comision-medica-rio-cuarto',
    metaTitle: 'Abogados Accidentes Laborales Río Cuarto | Capeletti',
    metaDescription: 'Abogados especialistas en accidentes laborales en Río Cuarto, Córdoba. Consulta gratis.',
  },
  {
    slug: 'san-francisco',
    nombre: 'San Francisco',
    provincia: 'Córdoba',
    provinciaSlug: 'cordoba',
    descripcion: 'San Francisco es el principal centro industrial del este cordobés, con fuerte actividad en metalurgia, agroindustria y manufactura.',
    poblacion: '75.000',
    sectorPrincipal: 'metalurgia, agroindustria, industria',
    cmLocal: 'CM — San Francisco',
    cmSlug: 'comision-medica-san-francisco',
    metaTitle: 'Abogados Laborales San Francisco Córdoba | Capeletti',
    metaDescription: 'Abogados especializados en ART y accidentes laborales en San Francisco, Córdoba. Metalurgia y agroindustria. Consulta gratis.',
  },
  {
    slug: 'villa-maria',
    nombre: 'Villa María',
    provincia: 'Córdoba',
    provinciaSlug: 'cordoba',
    descripcion: 'Villa María es un importante centro agroindustrial y de servicios del centro cordobés, con actividad en lácteos, cerámicas y construcción.',
    poblacion: '90.000',
    sectorPrincipal: 'lácteos, cerámica, construcción',
    cmLocal: 'CM — Villa María',
    cmSlug: 'comision-medica-villa-maria',
    metaTitle: 'Abogados Laborales Villa María Córdoba | Capeletti',
    metaDescription: 'Abogados para reclamos a la ART en Villa María, Córdoba. Agroindustria y construcción. Consulta gratis.',
  },
  {
    slug: 'bell-ville',
    nombre: 'Bell Ville',
    provincia: 'Córdoba',
    provinciaSlug: 'cordoba',
    descripcion: 'Bell Ville es un centro ganadero y agroindustrial del sur cordobés, con actividad en frigoríficos, tambos y campo.',
    poblacion: '35.000',
    sectorPrincipal: 'ganadería, agroindustria, frigoríficos',
    cmLocal: 'CM — Bell Ville',
    cmSlug: 'comision-medica-bell-ville',
    metaTitle: 'Abogados Laborales Bell Ville Córdoba | Capeletti',
    metaDescription: 'Abogados especializados en accidentes laborales en Bell Ville, Córdoba. Ganadería y agroindustria. Consulta gratis.',
  },
  {
    slug: 'villa-dolores',
    nombre: 'Villa Dolores',
    provincia: 'Córdoba',
    provinciaSlug: 'cordoba',
    descripcion: 'Villa Dolores combina turismo serrano, agroindustria y minería en el noroeste cordobés. Frecuentes accidentes en campo y construcción.',
    poblacion: '35.000',
    sectorPrincipal: 'turismo, agroindustria, minería',
    cmLocal: 'CM — Villa Dolores',
    cmSlug: 'comision-medica-villa-dolores',
    metaTitle: 'Abogados Laborales Villa Dolores Córdoba | Capeletti',
    metaDescription: 'Abogados para reclamos de ART en Villa Dolores, Córdoba. Agroindustria y turismo serrano. Consulta gratis.',
  },
  {
    slug: 'laboulaye',
    nombre: 'Laboulaye',
    provincia: 'Córdoba',
    provinciaSlug: 'cordoba',
    descripcion: 'Laboulaye es un centro ganadero del sur cordobés con actividad en agroindustria, frigoríficos y comercio rural.',
    poblacion: '25.000',
    sectorPrincipal: 'ganadería, agroindustria, comercio',
    cmLocal: 'CM — Laboulaye',
    cmSlug: 'comision-medica-laboulaye',
    metaTitle: 'Abogados Laborales Laboulaye Córdoba | Capeletti',
    metaDescription: 'Abogados especializados en accidentes laborales en Laboulaye, Córdoba. Ganadería y agroindustria. Consulta gratis.',
  },
];

export const PROVINCIAS_DATA: ProvinciaData[] = [
  {
    slug: 'santa-fe',
    nombre: 'Santa Fe',
    descripcion: 'Santa Fe es la provincia con mayor actividad industrial del interior argentino. Nuestro estudio tiene presencia en Rosario, la capital provincial, Reconquista, Rafaela y Venado Tuerto.',
    ciudades: CIUDADES_SANTA_FE,
    cmPrincipal: 'CM N° 2 — Rosario / CM N° 1 — Santa Fe Capital',
    metaTitle: 'Abogados Laborales Santa Fe | Capeletti Abogados',
    metaDescription: 'Abogados especializados en accidentes laborales y ART en Santa Fe. Rosario, capital, Reconquista y más ciudades. Consulta gratis.',
    contenidoHtml: `
<p class="leading-relaxed mb-5" style="color:rgba(11,24,58,0.7);">En Capeletti Abogados, nuestra conexión con Santa Fe es natural: es el corazón productivo donde la agroindustria y el cordón metalmecánico definen el ritmo del país. Operamos en esta provincia porque conocemos la complejidad de los accidentes en plantas aceiteras, terminales portuarias y talleres metalúrgicos. Desde siniestros en silos hasta lesiones por movimientos repetitivos en líneas de montaje, estamos presentes para que el trabajador santafesino reciba la indemnización que la ley le garantiza.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Sectores laborales más afectados en Santa Fe</h2>
<p class="leading-relaxed mb-4" style="color:rgba(11,24,58,0.7);">La siniestralidad en la provincia se concentra en sectores de alta exigencia física. La <strong style="color:#0b183a;">Agroindustria y Aceiteras</strong> registran accidentes graves por caídas en altura, atrapamientos en cintas transportadoras y explosiones de polvo en silos. La <strong style="color:#0b183a;">Industria Metalmecánica</strong>, pilar en ciudades como Rosario y Rafaela, presenta riesgos constantes de cortes, amputaciones y quemaduras por soldadura. El <strong style="color:#0b183a;">Transporte de Cargas</strong>, vinculado a la exportación de granos, sufre alto índice de accidentes viales. La <strong style="color:#0b183a;">Logística Portuaria</strong> expone a los estibadores a lesiones osteoarticulares severas por maniobras de carga en buques y barcazas.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Comisiones Médicas en Santa Fe</h2>
<ul class="space-y-3 ml-5 list-disc mb-5" style="color:rgba(11,24,58,0.7);">
  <li><strong style="color:#0b183a;">Comisión Médica N° 2 (Rosario):</strong> Centraliza los casos del sur provincial y el cordón industrial, siendo una de las de mayor actividad del país.</li>
  <li><strong style="color:#0b183a;">Comisión Médica N° 1 (Santa Fe Capital):</strong> Atiende los reclamos de la zona centro y norte de la provincia.</li>
  <li><strong style="color:#0b183a;">Delegaciones Rafaela y Venado Tuerto:</strong> Sedes estratégicas para descentralizar los trámites de los nodos productivos del interior santafesino.</li>
</ul>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Por qué necesitás un abogado local en Santa Fe</h2>
<p class="leading-relaxed mb-4" style="color:rgba(11,24,58,0.7);">Conocemos los criterios locales para la fijación de incapacidades y los Convenios Colectivos de Aceiteros, UOM o Camioneros, fundamentales para liquidar correctamente el Ingreso Base Mensual. La representación local te asegura que el cálculo de tu indemnización incluya todos los adicionales que ganás por convenio, evitando que la ART te pague de menos.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Casos que atendemos en Santa Fe</h2>
<ul class="space-y-3 ml-5 list-disc mb-5" style="color:rgba(11,24,58,0.7);">
  <li><strong style="color:#0b183a;">Accidentes en Plantas y Silos:</strong> Caídas, atrapamientos y exposición a atmósferas peligrosas.</li>
  <li><strong style="color:#0b183a;">Lesiones Metalúrgicas:</strong> Traumatismos por prensas, tornos y equipos pesados.</li>
  <li><strong style="color:#0b183a;">Enfermedades Profesionales:</strong> Hipoacusia por ruido industrial, asma ocupacional y hernias de disco.</li>
  <li><strong style="color:#0b183a;">Accidentes In Itinere:</strong> Choques en Circunvalación, Ruta 34 o Autopista Rosario-Santa Fe.</li>
  <li><strong style="color:#0b183a;">Rechazos de Siniestros:</strong> Accionamos cuando la ART alega que tu lesión es "preexistente" para no hacerse cargo.</li>
</ul>`,
  },
  {
    slug: 'neuquen',
    nombre: 'Neuquén',
    descripcion: 'Neuquén es la provincia patagónica con mayor crecimiento. La industria petrolera y la construcción concentran la mayor cantidad de accidentes laborales graves.',
    ciudades: CIUDADES_NEUQUEN,
    cmPrincipal: 'CM N° 9 — Neuquén Capital',
    metaTitle: 'Abogados Laborales Neuquén | Capeletti Abogados',
    metaDescription: 'Abogados especializados en ART, petróleo y accidentes laborales en Neuquén. CM local N°9. Consulta gratis y sin anticipos.',
    contenidoHtml: `
<p class="leading-relaxed mb-5" style="color:rgba(11,24,58,0.7);">En Capeletti Abogados, entendemos que la realidad laboral de Neuquén es única en el país. El motor de la provincia, impulsado por el desarrollo masivo de Vaca Muerta y la actividad hidrocarburífera, genera una dinámica de alta intensidad que no siempre respeta la integridad física de los trabajadores. Operamos en esta provincia porque sabemos que el sector petrolero, si bien ofrece salarios competitivos, conlleva riesgos extremos. Nuestro compromiso es garantizar que el trabajador neuquino no quede desprotegido ante el poder de las grandes operadoras y las ART.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Sectores laborales más afectados en Neuquén</h2>
<p class="leading-relaxed mb-4" style="color:rgba(11,24,58,0.7);">La economía neuquina se sostiene sobre pilares de alta siniestralidad. El <strong style="color:#0b183a;">sector Petrolero y Gasífero</strong> encabeza la lista: las tareas en torres de perforación, el manejo de químicos y las jornadas de "diagrama" extendidas suelen derivar en aplastamientos, quemaduras o patologías crónicas por esfuerzo. En segundo lugar, el <strong style="color:#0b183a;">Transporte y Logística</strong>, vital para el abastecimiento de los yacimientos, registra un alto índice de accidentes viales y lesiones osteoarticulares. También destaca la <strong style="color:#0b183a;">Construcción</strong>, vinculada a la infraestructura civil y petrolera, donde las caídas de altura y los golpes por objetos son frecuentes. Finalmente, los <strong style="color:#0b183a;">Servicios Petroleros</strong>, que incluyen mantenimiento y catering en campamentos, exponen a riesgos ergonómicos y accidentes in itinere.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Comisiones Médicas en Neuquén</h2>
<p class="leading-relaxed mb-3" style="color:rgba(11,24,58,0.7);">Para reclamar una indemnización, el paso obligado es la instancia administrativa ante la SRT. En la provincia contamos con sedes estratégicas:</p>
<ul class="space-y-3 ml-5 list-disc mb-5" style="color:rgba(11,24,58,0.7);">
  <li><strong style="color:#0b183a;">Comisión Médica N° 9 (Neuquén Capital):</strong> Tiene jurisdicción sobre la capital y zonas de influencia directa, concentrando el mayor volumen de expedientes por la cercanía a las sedes administrativas de las empresas.</li>
  <li><strong style="color:#0b183a;">Delegación Zapala:</strong> Fundamental para los trabajadores del centro de la provincia, evitando traslados innecesarios.</li>
  <li><strong style="color:#0b183a;">Delegación Plaza Huincul/Cutral-Có:</strong> Atiende los casos del corazón petrolero de la provincia.</li>
</ul>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Por qué necesitás un abogado local en Neuquén</h2>
<p class="leading-relaxed mb-4" style="color:rgba(11,24,58,0.7);">Tener representación con presencia en Neuquén no es solo una cuestión de cercanía — es una ventaja estratégica. Conocemos cómo funcionan las Comisiones Médicas locales y los tiempos de los juzgados laborales de la provincia. Fundamentalmente, entendemos los Convenios Colectivos de Trabajo específicos, como el de Petroleros Privados (CCT 644/12) o el de la Construcción (UOCRA), que tienen escalas salariales e ítems particulares que impactan directamente en el cálculo de tu indemnización. Un abogado que no conoce la liquidación de un recibo petrolero puede hacerte perder mucho dinero.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Casos que atendemos en Neuquén</h2>
<ul class="space-y-3 ml-5 list-disc mb-5" style="color:rgba(11,24,58,0.7);">
  <li><strong style="color:#0b183a;">Accidentes en Yacimiento:</strong> Explosiones, atrapamientos, golpes y caídas en pozos o plantas.</li>
  <li><strong style="color:#0b183a;">Accidentes In Itinere:</strong> Siniestros en las rutas 7, 22 y 151, frecuentes en el traslado a yacimientos.</li>
  <li><strong style="color:#0b183a;">Enfermedades Profesionales:</strong> Hernias de disco por cargas, hipoacusia por ruido de maquinaria y afecciones respiratorias.</li>
  <li><strong style="color:#0b183a;">Impugnación de Altas Médicas:</strong> Si la ART te dio el alta pero todavía tenés limitaciones, apelamos de inmediato.</li>
  <li><strong style="color:#0b183a;">Diferencias de Indemnización:</strong> Si el porcentaje de incapacidad es menor al real, vamos a juicio para que se te pague lo que corresponde.</li>
</ul>`,
  },
  {
    slug: 'rio-negro',
    nombre: 'Río Negro',
    descripcion: 'Río Negro combina actividades de turismo, fruticultura y construcción. Nuestra oficina en Neuquén capital cubre toda la región patagónica.',
    ciudades: CIUDADES_RIO_NEGRO,
    cmPrincipal: 'CM N° 18 — Viedma / CM N° 35 — General Roca / CM — Bariloche',
    metaTitle: 'Abogados Laborales Río Negro | Capeletti Abogados',
    metaDescription: 'Abogados para reclamos de ART en Río Negro. Bariloche, General Roca, Viedma y toda la provincia. Consulta gratis con especialista.',
    contenidoHtml: `
<p class="leading-relaxed mb-5" style="color:rgba(11,24,58,0.7);">En Río Negro, la producción frutícola y el turismo de alta montaña generan un escenario laboral de gran exigencia física. Capeletti Abogados brinda soporte en la provincia porque entendemos que el trabajador del Alto Valle o de la zona cordillerana enfrenta riesgos estacionales muy marcados. Ya sea en la cosecha de la pera y la manzana, en las cámaras frigoríficas o en los servicios turísticos de Bariloche, los accidentes ocurren y las ART suelen retacear la atención.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Sectores laborales más afectados en Río Negro</h2>
<p class="leading-relaxed mb-4" style="color:rgba(11,24,58,0.7);">La <strong style="color:#0b183a;">Fruticultura</strong> es el sector crítico, con accidentes recurrentes por caídas de escaleras durante la cosecha y lesiones por maquinaria agrícola. El sector de <strong style="color:#0b183a;">Empaque y Frigoríficos</strong> registra afecciones respiratorias por frío y lesiones por movimientos repetitivos en líneas de selección. El <strong style="color:#0b183a;">Turismo y Hotelería</strong>, central en Bariloche, presenta riesgos ergonómicos para mucamas y personal de cocina, además de accidentes en traslados de montaña. La <strong style="color:#0b183a;">Industria Extractiva</strong> en la zona este también aporta casos de siniestralidad por manejo de cargas pesadas.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Comisiones Médicas en Río Negro</h2>
<ul class="space-y-3 ml-5 list-disc mb-5" style="color:rgba(11,24,58,0.7);">
  <li><strong style="color:#0b183a;">Comisión Médica N° 18 (Viedma):</strong> Atiende la capital y la zona del Valle Inferior.</li>
  <li><strong style="color:#0b183a;">Comisión Médica N° 35 (General Roca):</strong> Sede central para todos los trabajadores del Alto Valle del Río Negro.</li>
  <li><strong style="color:#0b183a;">Delegación Bariloche:</strong> Para la zona andina, evitando que los trabajadores deban cruzar la provincia para una junta médica.</li>
</ul>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Por qué necesitás un abogado local en Río Negro</h2>
<p class="leading-relaxed mb-4" style="color:rgba(11,24,58,0.7);">El trabajo temporario o de temporada —típico en la fruta y el turismo— requiere un análisis legal especial para calcular la indemnización. En Capeletti Abogados sabemos cómo computar esos meses de actividad para que el cálculo del ingreso base sea justo. Conocemos los convenios de Fruticultura y Gastronómicos, y estamos habituados a la dinámica de las comisiones rionegrinas.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Casos que atendemos en Río Negro</h2>
<ul class="space-y-3 ml-5 list-disc mb-5" style="color:rgba(11,24,58,0.7);">
  <li><strong style="color:#0b183a;">Accidentes de Cosecha:</strong> Caídas de "bins" o escaleras, y heridas con herramientas de poda.</li>
  <li><strong style="color:#0b183a;">Lesiones por Frío:</strong> Afecciones en manos y pulmones por trabajo en cámaras frigoríficas.</li>
  <li><strong style="color:#0b183a;">Accidentes en Rutas del Valle:</strong> Siniestros en la Ruta 22 y rutas rurales de acceso a chacras.</li>
  <li><strong style="color:#0b183a;">Enfermedades de Columna:</strong> Muy frecuentes por la carga de cajones y posturas forzadas en tareas rurales.</li>
  <li><strong style="color:#0b183a;">Impugnación de Porcentaje:</strong> Si te dan un 0% y seguís con dolor, lo revertimos con fundamentos médicos.</li>
</ul>`,
  },
  {
    slug: 'entre-rios',
    nombre: 'Entre Ríos',
    descripcion: 'Entre Ríos tiene fuerte actividad agroindustrial y citrícola. Nuestro equipo atiende casos en Paraná, Concordia, Concepción del Uruguay y toda la provincia.',
    ciudades: CIUDADES_ENTRE_RIOS,
    cmPrincipal: 'CM N° 8 — Paraná / CM N° 34 — Concordia',
    metaTitle: 'Abogados Laborales Entre Ríos | Capeletti Abogados',
    metaDescription: 'Abogados especializados en accidentes laborales y ART en Entre Ríos. Paraná, Concordia y más. Consulta gratis.',
    contenidoHtml: `
<p class="leading-relaxed mb-5" style="color:rgba(11,24,58,0.7);">Entre Ríos combina una economía diversa donde la avicultura, la ganadería, la actividad citrícola y el turismo fluvial coexisten con sectores industriales en expansión. Capeletti Abogados opera en la provincia porque conocemos que los trabajadores rurales e industriales entrerrianos enfrentan riesgos específicos que las ART suelen minimizar. Nuestra presencia garantiza que cada trabajador tenga acceso a una defensa técnica sólida sin importar la distancia.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Sectores laborales más afectados en Entre Ríos</h2>
<p class="leading-relaxed mb-4" style="color:rgba(11,24,58,0.7);">La <strong style="color:#0b183a;">Avicultura</strong> concentra una parte importante de los accidentes de la provincia: manipulación de maquinaria en galpones, cargas repetitivas y exposición a gases. La <strong style="color:#0b183a;">Citricultura</strong> en la zona de Concordia y Federación genera lesiones por caídas en altura durante la cosecha y por movimientos repetitivos. El sector <strong style="color:#0b183a;">Cárnico y de Frigoríficos</strong> registra cortes y lesiones por frío. Finalmente, la <strong style="color:#0b183a;">Construcción e Infraestructura</strong> en Paraná y Concordia aporta casos de caídas y golpes frecuentes.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Comisiones Médicas en Entre Ríos</h2>
<ul class="space-y-3 ml-5 list-disc mb-5" style="color:rgba(11,24,58,0.7);">
  <li><strong style="color:#0b183a;">Comisión Médica N° 8 (Paraná):</strong> Sede principal que cubre la capital y zona centro de la provincia.</li>
  <li><strong style="color:#0b183a;">Comisión Médica N° 34 (Concordia):</strong> Atiende el norte entrerriano y la zona citrícola de Federación y Colón.</li>
  <li><strong style="color:#0b183a;">Comisión Médica N° 41 (Concepción del Uruguay):</strong> Cubre el sur de la provincia y la zona de Gualeguaychú.</li>
</ul>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Por qué necesitás un abogado local en Entre Ríos</h2>
<p class="leading-relaxed mb-4" style="color:rgba(11,24,58,0.7);">Los convenios colectivos de avicultores (UATRE) y frigoríficos tienen particularidades salariales que impactan directamente en el cálculo del ingreso base. Conocemos los juzgados laborales de Paraná y Concordia y estamos habituados a impugnar dictámenes de las CMs entrerrianas. Un abogado que no conoce esos convenios puede hacerte perder diferencias significativas en la liquidación final.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Casos que atendemos en Entre Ríos</h2>
<ul class="space-y-3 ml-5 list-disc mb-5" style="color:rgba(11,24,58,0.7);">
  <li><strong style="color:#0b183a;">Accidentes en Galpones Avícolas:</strong> Lesiones por maquinaria, cargas y exposición a gases.</li>
  <li><strong style="color:#0b183a;">Lesiones en Cosecha Citrícola:</strong> Caídas de escaleras y lesiones musculoesqueléticas por esfuerzo repetitivo.</li>
  <li><strong style="color:#0b183a;">Accidentes en Frigoríficos:</strong> Cortes, lesiones por frío y golpes con equipos de carga.</li>
  <li><strong style="color:#0b183a;">Accidentes In Itinere:</strong> Rutas nacionales 14 y 18, frecuentes en traslados rurales.</li>
  <li><strong style="color:#0b183a;">Enfermedades Profesionales:</strong> Hipoacusia, afecciones respiratorias y problemas lumbares.</li>
</ul>`,
  },
  {
    slug: 'buenos-aires',
    nombre: 'Buenos Aires',
    descripcion: 'Buenos Aires incluye CABA y el conurbano, con millones de trabajadores y alta actividad en construcción, salud, gastronomía y logística.',
    ciudades: CIUDADES_BUENOS_AIRES,
    cmPrincipal: 'CM N° 10 — CABA / CM N° 36 — La Plata / CM N° 38 — GBA',
    metaTitle: 'Abogados Laborales Buenos Aires | Capeletti Abogados',
    metaDescription: 'Abogados especializados en accidentes laborales y ART en Buenos Aires y CABA. Consulta gratis sin anticipos.',
    contenidoHtml: `
<p class="leading-relaxed mb-5" style="color:rgba(11,24,58,0.7);">La provincia de Buenos Aires, con su vasto cordón industrial y el dinamismo del GBA, presenta los índices de siniestralidad más altos del país. En Capeletti Abogados operamos en territorio bonaerense para asistir a trabajadores de la construcción, operarios de logística y empleados industriales que día a día enfrentan condiciones de riesgo. Estamos acá para nivelar la balanza frente a las ART, asegurando que ningún trabajador quede desamparado tras un siniestro.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Sectores laborales más afectados en Buenos Aires</h2>
<p class="leading-relaxed mb-4" style="color:rgba(11,24,58,0.7);">La <strong style="color:#0b183a;">Construcción</strong> es el sector con mayor riesgo en el GBA y La Plata, con frecuentes caídas de andamios y derrumbes. La <strong style="color:#0b183a;">Logística y Almacenamiento</strong>, concentrada en parques industriales de zona norte y oeste, registra múltiples lesiones de columna y accidentes con autoelevadores. La <strong style="color:#0b183a;">Industria Manufacturera</strong> (textil, plástica y alimenticia) presenta riesgos de atrapamientos y dermatitis químicas. El <strong style="color:#0b183a;">Reparto y Mensajería</strong> sufre una epidemia de accidentes viales que las ART intentan rechazar sistemáticamente.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Comisiones Médicas en Buenos Aires</h2>
<p class="leading-relaxed mb-3" style="color:rgba(11,24,58,0.7);">La provincia cuenta con una red amplia de comisiones para que no tengas que viajar largas distancias:</p>
<ul class="space-y-3 ml-5 list-disc mb-5" style="color:rgba(11,24,58,0.7);">
  <li><strong style="color:#0b183a;">CM N° 10 (CABA):</strong> Moreno 401, Ciudad Autónoma de Buenos Aires.</li>
  <li><strong style="color:#0b183a;">CM N° 36 (La Plata):</strong> Berisso, Ensenada, Brandsen y zona sur bonaerense.</li>
  <li><strong style="color:#0b183a;">CM N° 38 y delegaciones GBA:</strong> Morón, Ramos Mejía, San Martín, Lomas de Zamora, Quilmes, Ezeiza y más — cubriendo todo el conurbano.</li>
  <li><strong style="color:#0b183a;">CM N° 4 (Bahía Blanca) y N° 3 (Mar del Plata):</strong> Para el interior y la zona costera.</li>
</ul>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Por qué necesitás un abogado local en Buenos Aires</h2>
<p class="leading-relaxed mb-4" style="color:rgba(11,24,58,0.7);">La justicia laboral bonaerense tiene particularidades procesales únicas. Contar con abogados que litigan en los Tribunales de Trabajo de la provincia y manejan los convenios de UOCRA, Comercio o Camioneros es vital. Sabemos cómo impugnar los dictámenes médicos insuficientes en las comisiones del GBA y cómo acelerar los tiempos para que tu indemnización no se licue por la inflación.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Casos que atendemos en Buenos Aires</h2>
<ul class="space-y-3 ml-5 list-disc mb-5" style="color:rgba(11,24,58,0.7);">
  <li><strong style="color:#0b183a;">Accidentes en Obras:</strong> Caídas de altura, golpes por caída de objetos y electrocución.</li>
  <li><strong style="color:#0b183a;">Siniestros Viales Laborales:</strong> Accidentes en moto, camionetas de reparto y traslados al trabajo.</li>
  <li><strong style="color:#0b183a;">Enfermedades por Esfuerzo:</strong> Hernias, problemas lumbares crónicos y várices por bipedestación.</li>
  <li><strong style="color:#0b183a;">Rechazo de Enfermedad Profesional:</strong> Cuando la ART dice que tu dolencia no está en el listado oficial.</li>
  <li><strong style="color:#0b183a;">Bajas Médicas Prematuras:</strong> Obligamos a que la ART siga brindando rehabilitación si no estás recuperado.</li>
</ul>`,
  },
  {
    slug: 'cordoba',
    nombre: 'Córdoba',
    descripcion: 'Córdoba es la segunda provincia más grande del país. Alta actividad en industria automotriz, construcción y agroindustria con numerosos accidentes laborales.',
    ciudades: CIUDADES_CORDOBA,
    cmPrincipal: 'CM N° 5 — Córdoba Capital / CM N° 6 — Río Cuarto',
    metaTitle: 'Abogados Laborales Córdoba | Capeletti Abogados',
    metaDescription: 'Abogados especializados en accidentes laborales y ART en Córdoba. Consulta gratis, sin anticipos.',
    contenidoHtml: `
<p class="leading-relaxed mb-5" style="color:rgba(11,24,58,0.7);">Córdoba es la segunda provincia del país en actividad industrial y una de las más dinámicas en materia laboral. La industria automotriz, el sector sojero y la construcción generan un volumen significativo de accidentes que las ART no siempre cubren adecuadamente. En Capeletti Abogados operamos en Córdoba para que el trabajador cordobés tenga acceso a una defensa técnica especializada, con conocimiento real de las comisiones médicas locales y los convenios colectivos que rigen la provincia.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Sectores laborales más afectados en Córdoba</h2>
<p class="leading-relaxed mb-4" style="color:rgba(11,24,58,0.7);">La <strong style="color:#0b183a;">Industria Automotriz y Metalmecánica</strong> (FIAT, Renault, autopartistas) concentra un alto número de accidentes por prensas, tornos y manipulación de materiales pesados. El <strong style="color:#0b183a;">Sector Agropecuario y Sojero</strong> presenta accidentes con maquinaria agrícola y exposición a agroquímicos. La <strong style="color:#0b183a;">Construcción</strong>, en plena expansión en Córdoba Capital, Río Cuarto y Villa María, suma caídas de altura y golpes. Finalmente, el <strong style="color:#0b183a;">Transporte de Cargas</strong> por las rutas 9, 36 y 7 registra frecuentes accidentes viales con resultado de lesiones graves.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Comisiones Médicas en Córdoba</h2>
<ul class="space-y-3 ml-5 list-disc mb-5" style="color:rgba(11,24,58,0.7);">
  <li><strong style="color:#0b183a;">Comisión Médica N° 5 (Córdoba Capital):</strong> La más activa de la provincia, con jurisdicción sobre la zona metropolitana.</li>
  <li><strong style="color:#0b183a;">Comisión Médica N° 6 (Río Cuarto):</strong> Cubre el sur cordobés y la región agropecuaria de La Pampa limítrofe.</li>
  <li><strong style="color:#0b183a;">CM Villa María, Bell Ville, San Francisco:</strong> Nodos estratégicos para el interior industrial de la provincia.</li>
  <li><strong style="color:#0b183a;">CM Villa Dolores y Laboulaye:</strong> Para la zona serrana oeste y el sur-centro provincial.</li>
</ul>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Por qué necesitás un abogado local en Córdoba</h2>
<p class="leading-relaxed mb-4" style="color:rgba(11,24,58,0.7);">Los convenios de SMATA (automotrices), UOCRA (construcción) y UATRE (rural) tienen escalas salariales específicas que afectan directamente el cálculo del ingreso base. Conocemos los juzgados laborales de Córdoba Capital y el funcionamiento de las comisiones médicas del interior. Esa combinación de conocimiento técnico-legal y presencia local es lo que garantiza una liquidación justa.</p>

<h2 class="text-2xl font-bold mt-10 mb-4" style="color:#0b183a;">Casos que atendemos en Córdoba</h2>
<ul class="space-y-3 ml-5 list-disc mb-5" style="color:rgba(11,24,58,0.7);">
  <li><strong style="color:#0b183a;">Accidentes en Fábricas Automotrices:</strong> Lesiones por prensas, grúas y líneas de montaje.</li>
  <li><strong style="color:#0b183a;">Accidentes Agrícolas:</strong> Vuelcos de tractores, atrapamientos en cosechadoras y exposición a agroquímicos.</li>
  <li><strong style="color:#0b183a;">Caídas en Obra:</strong> Andamios, escaleras y superficies de trabajo inestables.</li>
  <li><strong style="color:#0b183a;">Enfermedades Profesionales:</strong> Hipoacusia en planta industrial, hernia de disco y lesiones por vibración.</li>
  <li><strong style="color:#0b183a;">Impugnación de Porcentaje:</strong> Si la CM cordobesa fijó un porcentaje bajo, lo revisamos con fundamentos médicos.</li>
</ul>`,
  },
];
